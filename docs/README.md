...
2. 메뉴의 갯수는 총 20개 이하여야한다. 20개를 넘을시에 예외를 발생시킨다.

3. 음료만 주문할시에, 주문할수가 없다. 이때 예외를 발생시킨다.

4. 메뉴의 갯수는 1이상의 숫자여야한다. 만약 메뉴가 숫자가 아닐시에 예외를 발생시킨다.

5. 메뉴의 총 갯수는 20개 이하여야하니, 만약 1개라도 20개를 넘어간다면 예외를 발생시킨다.

6. 메뉴 갯수 안에 공백이 포함되어있거나, 앞 뒤가 공백이 들어가있을 경우 예외를 발생시킨다.

7. 메뉴 형식은 무조건 "메뉴-갯수", 그리고 ","로 구분한다. 해당 메뉴의 형식이 아닐시에 예외를 발생시킨다.

8. 메뉴 주문은 중복을 포함하면 안된다. 만약 중복을 포함했을 시에 예외를 발생시킨다.

9. 메뉴 총 주문 금액은 최소 10,000원 이상이어야한다. 이 조건을 충족 못할 시에 예외가 발생한다.
